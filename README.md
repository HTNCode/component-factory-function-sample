...
### ただし、モーダルのようなケースなら問題なさそう

- モーダルのように、子コンテンツの表示位置が親コンテンツの「外側」で良いケースなら上記の自動挿入で問題なさそう

```tsx
<Hoist>
  <div>親メニュー（ドロップダウン）</div>
  <Hoist>
    {/* 確認モーダル - 画面全体に被さるので位置は関係ない */}
    <Modal>本当に削除しますか？</Modal>
  </Hoist>
</Hoist>

// childContent が親の「外側」に表示されてもモーダルとかであれば問題ない
```

### 解決策: 子メニューはHoistのchildren内に直接配置

- あくまで子メニューが親メニューの「内側」に表示されるケースでは、HoistのネストやProviderのネストは使わずに以下のように実装することになる
